...
function doSearch(context) {
	...
	jQuery.each(inputs, function(index, value) {
		if ("" != jQuery(this).val()) {
			sysc.push({
				"coldType" : jQuery(this).attr("coldType"),
				"colsName" : jQuery(this).attr("colsName"),
				"colsDesc" : jQuery(this).val()
			});
		}
	});
	_total=tableRows;
		...
		if(userIndex==""||userIndex==null){
			...
			column.push({
	    		...
	    		formatter:function otherFormatter(value, row, index){
	    			...
	    			var f = value
	    			if(userIcon==null||userIcon==""){
	    				 f = '<a class="text-blue" title="'+value+'" userIndex="'+userIndex+'" onclick="forwordYjcx(\''+value+'\',\''+userIndex+'\')">'+value+' </a> ';
	    			}else{
	    				 f = f = '<i class="'+userIcon+' text-blue" title="'+value+'" userIndex="'+userIndex+'" onclick="forwordYjcx(\''+value+'\',\''+userIndex+'\')" id="sfzh_search"></i> ';
	    			}
	    			return f;
	    		},
			});
...
function forwordYjcx(value,userIndex){
	var paramList = [];
	paramList.push({
		"userIndex" : userIndex,
		"value" : value,
