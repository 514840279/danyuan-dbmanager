package com.shumeng.application.softm.sysmenu.dao;
...
public interface SysMenuDao extends JpaRepository<SysMenuInfo, String> {
	...
	public void updateSysMenuInfoName(@Param("parentsId") String parentsId, @Param("sort") int sort,
	...
	@Query("SELECT DISTINCT t FROM SysMenuInfo t " + "WHERE t.uuid IN("
			+ "	SELECT a.id.menuId FROM SysRolesJurisdictionInfo a " + "	WHERE a.id.roleId IN ( "
			+ "		SELECT b.rolesId FROM SysUserRolesInfo b " + "		WHERE b.userId IN ( "
			+ "			SELECT uuid FROM SysUserBaseInfo c " + "			WHERE c.userName=:userName " + "		) "
			+ "		AND b.checked='1' " + "	) " + "	AND a.checked='1' " + ") " + "AND t.parentsId = :uuid "
			+ " order by t.sort ")
